...
def main():
    ...
    file_ali1 = 'PF00005.22.hmm'
    file_ali2 = 'PF00528.17.hmm'
    file_out = 'PF00005.22.hmm-PF00528.17.hmm.zscores.inter'
    
    cmd = ["../cointerfaces.pl", "-hmm1", file_ali1, "-hmm2", file_ali2]
...
#    subprocess.run(cmd, stdout = file_stdout, stderr = file_stderr)
    ...
    if not (os.path.isfile(file_out) and os.path.getsize(file_out)):
        exit("ERROR: The cointerfaces protocol failed, output file with APC zscores not found.")
    
    num_positions = (137, 185)
    
    # Load the stored result and computed one and compare both
    file_old = "PF00005.22.hmm-PF00528.17.hmm.zscores.inter.old"
...
#        os.remove(file_out + ".err")
        ...
        print("Test partially successful. The APC zscores were computed but some minor difference were detected. This might be due to the use of a different pseudorandom number algorithm (MersenneTwister was used for the stored results). Compare top contact predictions in files test/mend_scores.old and test/mend_scores.new. If the both contain the same order of pair of positions with small numeric differences, it should be right.")
    else:
